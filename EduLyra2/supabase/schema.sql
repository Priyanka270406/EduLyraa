...
-- EduLyra Industry opportunity + certification extension (additive migration)
-- Preserves existing tables/data and adds the complete Internship / Workshop / Training lifecycle.
do $$
begin
  if not exists (select 1 from pg_enum where enumtypid='public.opportunity_type'::regtype and enumlabel='Workshop') then
    alter type public.opportunity_type add value 'Workshop';
  end if;
end $$;
...
create table if not exists public.opportunity_registrations (
  ...
  completed_at timestamptz,
  criteria_satisfied boolean not null default false,
...
create table if not exists public.issued_certificates (
  ...
  recipient_type public.app_role not null check(recipient_type in ('student','faculty')),
...
create index if not exists idx_opportunity_reg_user on public.opportunity_registrations(user_id);
create index if not exists idx_opportunity_reg_opportunity on public.opportunity_registrations(opportunity_id);
create index if not exists idx_training_completion_learner on public.training_completions(learner_id);
create index if not exists idx_issued_certificates_id on public.issued_certificates(certificate_id);
create index if not exists idx_issued_certificates_recipient on public.issued_certificates(recipient_id);

alter table public.opportunity_registrations enable row level security;
alter table public.training_completions enable row level security;
alter table public.issued_certificates enable row level security;

create policy "opportunity_registrations_learner_manage" on public.opportunity_registrations
for all to authenticated
using (user_id = auth.uid())
with check (user_id = auth.uid());

create policy "opportunity_registrations_industry_view" on public.opportunity_registrations
for select to authenticated
using (exists (
...
create policy "opportunity_registrations_industry_update" on public.opportunity_registrations
...
create policy "training_completions_learner_manage" on public.training_completions
for all to authenticated
using (learner_id = auth.uid())
with check (learner_id = auth.uid());

create policy "training_completions_industry_view" on public.training_completions
for select to authenticated
using (exists (
  select 1 from public.opportunities o
  where o.id = opportunity_id and o.owner_id = auth.uid()
...
create policy "issued_certificates_public_verify" on public.issued_certificates
for select to anon, authenticated
using (true);

create policy "issued_certificates_industry_issue" on public.issued_certificates
for insert to authenticated
with check (
  issued_by = auth.uid()
  and recipient_type in ('student','faculty')
  and exists (
    select 1 from public.opportunities o
  ...
  and exists (
    select 1 from public.opportunity_registrations r
    where r.opportunity_id = opportunity_id
      and r.user_id = recipient_id
      and r.status = 'Completed'
      and r.criteria_satisfied = true
  )
);

create policy "issued_certificates_recipient_update_none" on public.issued_certificates
for update to authenticated
