...
import {
  ...
  getMessagesForUser,
  allUsers,
  markAllNotifications, registerForOpportunity, getOpportunityRegistration, completeTrainingOpportunity,
} from "../services/store";
...
export function Opportunities(){
 const u:any=getMyData(); const skills:string[]=u?.profile?.currentSkills||[]; const gaps:string[]=u?.assessment?.gaps||[]; const target=String(u?.profile?.desiredJobRole||u?.profile?.careerGoal||'').toLowerCase(); const [q,setQ]=useState(''); const [type,setType]=useState('All'); const today=new Date().toISOString().slice(0,10); const ops=getOpportunities().filter((o:any)=>(o.status||'Published')==='Published'&&(!o.deadline||o.deadline>=today));
 ...
 const f=useMemo(()=>{const query=alias(q).toLowerCase().trim();const audienceOk=(o:any)=>{const a=String(o.audience||'Both').toLowerCase();return a.includes('both')||(u?.role==='student'&&a.includes('student'))||(u?.role==='faculty'&&a.includes('faculty'));};return ops.filter(o=>audienceOk(o)&&(type==='All'||o.type===type)&&(!query||`${o.title} ${o.company} ${o.skills.join(' ')} ${o.description} ${o.location}`.toLowerCase().includes(query))).map(o=>({...o,match:relevance(o)})).sort((a,b)=>b.match-a.match)},[q,type,ops,u]);
...
export function OpportunityDetails({ id }: { id: string }) {
 const nav=useNavigate(); const user:any=getMyData(); const role=user?.role; const base=role==="faculty"?"/faculty/opportunities":"/student/opportunities";
 const opportunity:any=getOpportunities().find((item)=>item.id===id);
 const participation:any=getOpportunityRegistration(id,user?.id);
 const [refresh,setRefresh]=useState(0);
 const [saved,setSaved]=useState(!!user?.profile?.savedOpportunities?.includes(id));
 if(!opportunity)return <div className="card p-8">Opportunity not found.</div>;
 const engagement=opportunity.engagement||(["Workshop","Training"].includes(opportunity.type)?"Registration":"Application");
 const handleParticipate=()=>{try{registerForOpportunity(opportunity);setRefresh(x=>x+1)}catch(e:any){alert(e?.message||"Unable to participate.")}};
 const handleComplete=()=>{try{completeTrainingOpportunity(opportunity.id);setRefresh(x=>x+1)}catch(e:any){alert(e?.message||"Unable to complete training.")}};
 const handleSave=()=>{if(saved){removeSavedOpportunity(opportunity.id);setSaved(false)}else{saveOpportunity(opportunity);setSaved(true)}};
 const status=participation?.status;
 return <div className="mx-auto max-w-4xl space-y-5">
  <button className="btn-secondary" onClick={()=>nav(base)}>← Back to opportunities</button>
  <div className="card p-7">
   <div className="flex flex-wrap items-start justify-between gap-4"><div><span className="badge bg-cyan-50 text-cyan-700">{opportunity.type}</span><h1 className="mt-3 text-3xl font-black">{opportunity.title}</h1><p className="mt-1 text-slate-500">{opportunity.company} · {opportunity.location} · {opportunity.workMode}</p></div><MatchScore value={opportunity.match||0}/></div>
   <p className="mt-6 leading-7 text-slate-600">{opportunity.description}</p>
   <div className="mt-6 grid gap-4 sm:grid-cols-2">{[
    ["Eligibility",opportunity.eligibility],["Target audience",opportunity.audience||"Both"],["Deadline",opportunity.deadline||opportunity.registrationDeadline||"Not specified"],["Duration",opportunity.duration||"Not specified"],["Start / end",opportunity.startDate?`${opportunity.startDate}${opportunity.endDate?` → ${opportunity.endDate}`:""}`:"Not specified"],["Required skills",(opportunity.skills||[]).join(", ")||"Not specified"]
   ].map(([label,value])=><div className="rounded-xl bg-slate-50 p-4" key={label}><p className="text-xs text-slate-400">{label}</p><p className="mt-1 text-sm font-semibold">{value}</p></div>)}</div>
   {opportunity.type==="Training"&&<div className="mt-5 rounded-2xl border border-violet-100 bg-violet-50 p-5"><h2 className="font-black">Certification eligibility</h2><p className="mt-1 text-sm text-slate-600">Registration → completion → completion criteria satisfied → Industry certificate eligibility.</p><p className="mt-2 text-sm font-bold">{status==="Completed"?"✓ Completed — eligible for certification":status?"Status: "+status:"Register to begin"}</p></div>}
   {(role==="student"||role==="faculty")&&<div className="mt-6 flex flex-wrap gap-3">
    {!participation?<button className="btn-primary" onClick={handleParticipate}>{engagement}</button>:<span className="badge bg-emerald-50 text-emerald-700 px-4 py-2">{engagement==="Registration"?`✓ ${status}`:`✓ ${status}`}</span>}
    {opportunity.type==="Training"&&status&&status!=="Completed"&&<button className="btn-secondary" onClick={handleComplete}>Mark training completed</button>}
    <button className={`btn-secondary ${saved?"bg-violet-100 text-violet-700":""}`} onClick={handleSave}><Bookmark size={16} fill={saved?"currentColor":"none"}/>{saved?"Unsave Opportunity":"Save Opportunity"}</button>
   </div>}
  </div>
 </div>;
}
