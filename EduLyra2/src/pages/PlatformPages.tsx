...
export function ChallengeHub(){const [applied,setApplied]=useState<string[]>([]);const challenges: [string,string,string,string[],string][]=[['c1','AI Customer Support Analytics Platform','NovaTech Labs',['Python','React','PostgreSQL','Machine Learning'],'6 weeks'],['c2','Campus Energy Optimization','AeroStack',['Python','SQL','Cloud','Data Analytics'],'8 weeks'],['c3','Accessible Learning Assistant','EduInnovate Network',['React','Node.js','AI / ML'],'5 weeks']];return <div className="space-y-6"><Title title="Industry Challenge Hub" subtitle="Real-world problems become team projects, faculty mentorship and verified industry experience."/><div className="grid gap-4">{challenges.map(c=><Card className="p-6" key={c[0]}><div className="flex flex-wrap justify-between gap-4"><div><span className="badge bg-violet-50 text-violet-700">Industry Challenge</span><h2 className="mt-2 text-xl font-black">{c[1]}</h2><p className="mt-1 text-sm text-slate-500">{c[2]} · {c[4]}</p></div><Button disabled={applied.includes(c[0])} onClick={()=>setApplied(v=>[...v,c[0]])}>{applied.includes(c[0])?'Application submitted':'Apply / Form Team'}</Button></div><div className="mt-5 flex flex-wrap gap-2">{(c[3] as string[]).map(s=><span className="skill-chip" key={s}>{s}</span>)}</div><div className="mt-5 grid gap-2 md:grid-cols-4 text-xs"><div className="rounded-xl bg-slate-50 p-3">1. Student team</div><div className="rounded-xl bg-slate-50 p-3">2. Faculty mentor</div><div className="rounded-xl bg-slate-50 p-3">3. Industry review</div><div className="rounded-xl bg-slate-50 p-3">4. Verified experience</div></div></Card>)}</div></div>}
...
export function PassportVerification({id}:{id:string}){
 ...
 if(certificate)return <div className="mx-auto max-w-2xl space-y-6"><Title title="Certificate Verification" subtitle="Public verification record for an EduLyra-issued certificate."/><Card className="p-7"><div className="flex flex-wrap items-center justify-between gap-6"><div><span className="badge bg-emerald-50 text-emerald-700">✓ {certificate.verified?'Verified':'Pending'}</span><h2 className="mt-3 text-2xl font-black">{certificate.name}</h2><p className="mt-1 text-sm text-slate-500">Issued to {certOwner?.name||'EduLyra learner'} · {certificate.recipientType==='faculty'?'Faculty / Academician':'Student'}</p><p className="mt-4 text-sm">Certificate ID: <b>{certificate.id}</b></p><p className="mt-1 text-sm">Issuer: <b>{certificate.issuer}</b> · Issued: <b>{certificate.issuedOn}</b></p><p className="mt-1 text-sm">Program: <b>{certificate.trainingTitle||certificate.name}</b></p></div><img className="h-40 w-40 rounded-xl border p-2" alt="Certificate verification QR" src={`https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${payload}`}/></div><div className="mt-6 rounded-2xl bg-emerald-50 p-4 text-sm text-emerald-800">This certificate is automatically linked to the learner's EduLyra portfolio and can be checked using its unique certificate ID.</div></Card></div>;
